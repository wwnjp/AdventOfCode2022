...
const i = (require('fs')).readFileSync('./inputs/4.txt', 'utf-8').trim().split('\n')
   .forEach(_ => {
      ...
      // we can compare the ranges of numbers, so just look at if a >= x && b <= y
      // or the other way around (x >= a && y <= b)
      if (((s[0] >= s[2]) && (s[1] <= s[3])) || 
          ((s[2] >= s[0]) && (s[3] <= s[1]))) {
         numOverlaps1++;
      }

      // Part 2: Look for *any* overlap, which is even easier,
      // Just look to see if x <= b, but also make sure y >= a
      if ((s[2] <= s[1]) && (s[3] >= s[0])) {
         numOverlaps2++;
      }
   });
...
console.log('PART 1', numOverlaps1);
console.log('PART 2', numOverlaps2);
