...
const i = (require('fs')).readFileSync('./inputs/3.txt', 'utf-8').trim().split('\n')
   .map(_ => {
      // I would split the string in half and compare each string,
      // but this approach of just iterating through seemed algorithmically
      // faster
   ...
   // Funky way of mapping the numbers:
   // ASCII Codes are A-Z[\]^_a-z, A = 65, a = 97
   // if we subtract 96, we get a-z mapped to the right values, 
   // leaving A-Z in the range of -31 - -5
   // so we just check if the value is < 0, add 58 (31 + 27) to bring us to
   // 27 - 52!
